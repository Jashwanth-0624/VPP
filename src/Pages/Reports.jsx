...
import { Report } from "../entities/all.js";
...
export default function Reports() {
  ...
  const downloadReport = (report) => {
    ...
    const fileBase = `vpp-report-${report.start_date}-${report.end_date}`;
    const formatSelected = report.format || 'pdf';

    if (formatSelected === 'csv') {
      exportCSV(rows, fileBase);
      return;
    }
    if (formatSelected === 'xlsx') {
      exportExcel(rows, fileBase);
      return;
    }
    // PDF
    const [titleRow] = rows;
    const meta = rows.slice(1, 5).map(r => r.join(' ')).join('<br/>');
    const tableRows = rows.slice(5);
    const tableHtml = `<table><tr>${tableRows[0].map(h => `<th>${h}</th>`).join('')}</tr>${tableRows.slice(1).map(r => `<tr>${r.map(c => `<td>${c}</td>`).join('')}</tr>`).join('')}</table>`;
    exportPDF(titleRow[0], [
